...
class SingleChoiceQuestion(Question):
    ...
    def to_display(self) -> str:
        return f"[{index_label(len(self.options))}] 单选: {self.title}"
...
class FillInQuestion(Question):
    ...
    def to_display(self) -> str:
        return f"[填空{self.blanks}空] {self.title}"
...
class FileTaskQuestion(Question):
    ...
    def to_display(self) -> str:
        return f"[文件] {self.title} ({len(self.deliverables)}个文件)"
...
def index_label(option_count: int) -> str:
    letters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
    return "".join(letters[:option_count])
...
def generate_groups_info(questions: List[Question]) -> str:
    ...
    for q in questions:
        ...
        group = groups.setdefault(qtype, {"count": 0, "score": 0})
        group["count"] += 1
        group["score"] += q.score
    lines = []
    for idx, (qtype, data) in enumerate(groups.items(), start=1):
        lines.append(
            f"Group {idx}: type={qtype}, count={data['count']}, total_score={data['score']}"
        )
    return "\n".join(lines) + "\n"
